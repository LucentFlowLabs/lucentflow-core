...
**Schema alignment (v1.2 / Flyway V1):** The `whale_transactions` table exposes analyzer outputs including `risk_score`, `risk_reasons`, `rug_risk_level`, `execution_status` (`SUCCESS` | `REVERTED`), and genesis fields `funding_source_address` / `funding_source_tag`. The `sync_status` table (ID 1 Protocol) exposes heartbeat metrics including `chain_head_block`, `last_scanned_block`, `block_lag`, and `blocks_per_second`.
...
* Audit Note: This feed is risk-centric by design and must include both `risk_score` and `execution_status` to surface reverted high-risk activity.
...
```sql
SELECT
    COALESCE(to_address_tag, '👤 ' || LEFT(to_address, 8) || '...') AS "Entity",
    to_address AS "Address",
    ROUND(SUM(value_eth)::numeric, 2) || ' ' || COALESCE(token_symbol, 'ETH') AS "Total Inflow",
    SUM(value_eth) AS "raw_value",
    COALESCE(token_symbol, 'ETH') AS "asset_type",
    COUNT(*) AS "Tx Count",
...
```
* Visualization: Table
* Visual Note (Hidden Columns Strategy):
  * Hide `raw_value` and use it for correct numerical sorting (descending).
  * Hide `asset_type` and use it to drive "Highlight Whole Row" rules (e.g., Blue for ETH, Green for USDC).
